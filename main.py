...
class User(db.Model):
    ...
    def update_info(self, idi=None, update_data=None):
        ...
        if idi:
            ...
            if new_username:
                if User.query.filter_by(username=new_username).first() and update_user.user_name != new_username:
                    return jsonify({"message": "User with this data already exist"}, 404)
                update_user.user_name = new_username
...
class Goods(db.Model):
    ...
    def delete_goods_from_db(self, id_of_g=None, tag=None):
        # delete goods from db by his `id`
        if id_of_g:
            id = id_of_g.get('id')
            delete_goods = Goods.query.filter_by(id=id).first()
...
class GoodsController(object):
    ...
    def create(self, goods_data=None):
        ...
        if not self.model_goods.description or not self.model_goods.tag or not self.model_goods.price\
                or not self.model_goods.amount or not current_user:
            return jsonify({"message": "Invalid input"}, 400)
        else:
    ...
    def delete(self, id_of_g=None, tag=None, user_id=None):
        return self.model_goods.delete_goods_from_db(id_of_g=id_of_g, tag=tag)
...
def update_user(current_user):
    data = request.get_json()
    return UserController().update(current_user.id, data)
...
def delete_goods():
    ...
    return GoodsController().delete(identifier, current_tag)
