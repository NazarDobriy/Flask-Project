...
class GoodsController(object):
    ...
    def delete(self, id_of_g=None, tag=None):
...
# http://127.0.0.1:5000/UserView?id=1
@app.route('/UserView', methods=['GET'])
def read_user():
    idi = request.args.get('id')
    user_controller = UserController()
    return user_controller.read(idi)
...
def update_user():
    ...
    user_controller = UserController()
    return user_controller.update(idi, new_login, new_password, new_username)
...
# http://127.0.0.1:5000/GoodsCreate?tag=Mazda
...
# http://127.0.0.1:5000/GoodsByUser?user_id=1
@app.route('/GoodsByUser', methods=['GET'])
def goods_by_user():
    goods_data = request.args.get('user_id')
    read_goods = GoodsController().all_goods_by_user_id(goods_data)
    return read_goods
